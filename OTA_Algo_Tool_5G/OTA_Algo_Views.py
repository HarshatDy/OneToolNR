...
def ota_algo_calculation(request):
    if request.method != "POST":
        ...
        def get_start_time(filename):
            print(filename)
            with filename as src_file:
                line_1 = src_file.readline()
                line = line_1.read().decode('utf-8')
                print(type(line) )
                print("=======================================")
        ...
        class Graph:
            ...
            def parseFile(self):
                with self.config.filename as src_file:
            ...
            def plot(self, ueid):
                if self.config.plot_type==1:
                    ...
                    for plotItem in self.ueDict[ueid].plotDict.keys():
                        ...
                        print(len(self.ueDict[ueid].plotDict[plotItem]))
                        print(self.ueDict[ueid].plotDict[plotItem])
                        x,y = zip(*sorted(self.ueDict[ueid].plotDict[plotItem].items()))
                        # plt.plot(x, y, label = plotItem, linewidth = 2,marker = '.', markersize = 4)
